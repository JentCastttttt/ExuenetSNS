...
- (void)setState:(PRState)state animated:(BOOL)animated{
    ...
    if (_state != state) {
        ...
        if (_state == kPRStateLoading) {    //Loading
            ...
            _loading = YES;
            if (self.isAtTop) {
                _stateLabel.text = NSLocalizedString(@"正在刷新，请稍后(^_^)", @"");
            } else {
                _stateLabel.text = NSLocalizedString(@"正在加载，请稍后(^_^)", @"");
            }
            ...
            if (self.isAtTop) {
                _stateLabel.text = NSLocalizedString(@"松开后刷新哦⊙_⊙", @"");
            } else {
                _stateLabel.text = NSLocalizedString(@"释放加载更多哦⊙_⊙", @"");
            }
            ...
            if (!self.isAtTop) {    //footer
                _arrow.hidden = YES;
                _stateLabel.text = NSLocalizedString(@"没有了哦⊙_⊙", @"");
            }
        }
    }
}
